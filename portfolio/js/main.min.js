...
return deferred?deferred.promise($this):$this;};var Grid=(function(){var $grid=$('*#og-grid'),$items=$grid.children('li'),current=-1,previewPos=-1,scrollExtra=0,marginExpanded=10,$window=$(window),winsize,$body=$('html, body'),transEndEventNames={'WebkitTransition':'webkitTransitionEnd','MozTransition':'transitionend','OTransition':'oTransitionEnd','msTransition':'MSTransitionEnd','transition':'transitionend'},transEndEventName=transEndEventNames[Modernizr.prefixed('transition')],support=Modernizr.csstransitions,settings={minHeight:500,speed:350,easing:'ease'};function updateList(){$items=$grid.children('li');saveItemInfo(true);}
function init(config){settings=$.extend(true,{},settings,config);$grid.imagesLoaded(function(){saveItemInfo(true);getWinSize();initEvents();});}
...
current=this.$item.index();var $itemEl=this.$item.children('a'),eldata={href:$itemEl.attr('href'),largesrc:$itemEl.data('largesrc'),title:$itemEl.data('title'),description:$itemEl.data('description'),exif:$itemEl.data('exif'),buttontext:$itemEl.data('buttontext'),collectionName:$itemEl.data('collection'),isVideo:$itemEl.data('is-video')};this.$title.html(eldata.title);this.$description.html(eldata.description);if(eldata.exif){this.$exifInfo.html(eldata.exif);this.$exifInfo.show();}else this.$exifInfo.hide();if(eldata.collectionName){this.$collectionName.html(eldata.collectionName);this.$collectionName.show();}else this.$collectionName.hide();if(eldata.buttontext)this.$href.text(eldata.buttontext);if(eldata.href&&eldata.buttontext){this.$href.attr('href',eldata.href);this.$href.show();}
else{this.$href.hide();}
var self=this;if(typeof self.$largeImg!='undefined'){self.$largeImg.remove();}
if(self.$fullimage.is(':visible')){this.$loading.show();if(eldata.isVideo){$('<iframe allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen/>').on("load",function(){self.$loading.hide();}).attr('src',eldata.largesrc).fadeIn(350).appendTo(self.$fullimage);}else{$('<img/>').on("load",function(){var $img=$(this);self.$loading.hide();self.$fullimage.find('img').remove();self.$largeImg=$img.fadeIn(350);self.$fullimage.append(self.$largeImg);}).attr('src',eldata.largesrc);}}else{if(eldata.isVideo){$('<a target="_blank"/>').attr("href",eldata.largesrc).appendTo(self.$title);}}},open:function(){setTimeout($.proxy(function(){this.setHeights();this.positionPreview();},this),25);},close:function(){var self=this,onEndFn=function(){if(support){$(this).off(transEndEventName);}
...
return{init:init,addItems:addItems,updateList:updateList};})();document.addEventListener("DOMContentLoaded",function(){var lazyloadImages=document.querySelectorAll("img.lazy");if("IntersectionObserver"in window){var imageObserver=new IntersectionObserver(function(entries,observer){entries.forEach(function(entry){if(entry.isIntersecting){var image=entry.target;$(image).on("load",function(){image.classList.remove("lazy");image.style.removeProperty("width");}).attr('src',image.dataset.src);imageObserver.unobserve(image);}});});lazyloadImages.forEach(function(image){imageObserver.observe(image);});}else{lazyloadImages.forEach(function(img){img.src=img.dataset.src;img.classList.remove('lazy');img.style.removeProperty("width");});}})